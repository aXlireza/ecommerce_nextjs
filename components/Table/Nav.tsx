...
const TableNav: React.FC<Nav> = ({ rateActive=false, title, date, rateFunc, upToDate=true }) => {
    ...
    <section className="relative w-full flex items-center bg-white justify-between overflow-hidden samen-border">
			...
			<div className='flex flex-row items-center'>
				<div className={`z-10 bg-${upToDate ? 'samen-orange' : 'samen-gray'} w-fit h-[50px] flex items-center justify-center pr-5`}>
					<p className="text-center font-semibold samen-text text-sm sm:text-base">{title}</p>
				</div>
				{/* triangle */}
				<span className={`w-0 h-0 
					border-t-[50px] border-t-transparent
					border-r-[40px] border-r-${upToDate ? 'samen-orange' : 'samen-gray'}
