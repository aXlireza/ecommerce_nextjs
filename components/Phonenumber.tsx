...
const Phonenumber: React.FC<Props> = ({ number }) => {
	return (
		<Link href={`tel:+98${convertToEng(number.slice(1).replaceAll(' ', '').replaceAll('-', ''))}`} className="text-black dark:text-white flex flex-row items-center gap-2">
			...
			<div className='relative'>
				...
				<FiPhone className="bg-amber-500 rounded-full fill-black stroke-none p-2 w-10 h-10" />
			</div>
		</Link>
	)
}
