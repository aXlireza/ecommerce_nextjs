...
const Footer: React.FC<Props> = ({ logo_alt, tel, email, workSchedule, address, lists }) => {
	return (
    <footer className='py-3 px-10 md:px-24 flex flex-col justify-center bg-white dark:bg-black px-10 lg:px-24 py-5 gap-4'>
      ...
      <Image
        ...
        height={20}
      />
      <div className='text-black dark:text-white flex flex-col justify-between lg:flex-row w-full gap-10'>
        ...
        <div className='flex flex-row justify-between w-max gap-10'>
          {lists.map((list, key) => (
            <ul key={key} className='text-md text-slate-700 dark:text-slate-200'>
              <li className='text-lg font-bold text-slate-900 dark:text-slate-100'><Link href={list.head.link}>{list.head.title}</Link></li>
